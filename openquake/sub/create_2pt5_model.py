...
def create_2pt5_model(in_path, out_path, maximum_sampling_distance=25.):
    ...
    print("maximum sampling distance is ", maximum_sampling_distance)
    # drop profiles where length is less than the sampling distance
    L2 = {k: v for k, v in lengths.items() if v >= maximum_sampling_distance}
    sps2 = {key: sps[key] for key in L2}


    # Resampled profiles
    rsps = get_interpolated_profiles(sps2, lengths, number_of_samples)
