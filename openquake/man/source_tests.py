...
def occurence_table(path_oq_input, path_to_subcatalogues, 
                    minmag, minyear, maxyear, N, src_ids,
                    src_str='src_', sconv = DEFAULT):
    '''
    ...
    '''
    table, agr_cats, agrs, bgrs, miocs = [],[],[],[],[]
    maocs, perc_16s, perc_84s, obss, pf, sids = [],[],[],[],[],[]
    time_span = maxyear - minyear

    for src_id in sorted(src_ids):

        fname_src = os.path.join(path_oq_input, f"{src_str}{src_id}.xml")
        ...
        perc_16 = np.percentile(num_occ_per_time_span, 95)
        perc_84 = np.percentile(num_occ_per_time_span, 5)
    
        perc_16 = poisson.ppf(0.95, rate*time_span)
        perc_84 = poisson.ppf(0.05, rate*time_span)
        ...
        agr_cats.append(agr_cat)
        sids.append(src_id)

        agrs.append(agr); bgrs.append(bgr)
        miocs.append(mioc); maocs.append(maoc);
        perc_16s.append(perc_16); perc_84s.append(perc_84)
        obss.append(obs); pf.append(pss)

    heads = ["Zone", "agr_cat", "agr", "bgr", "min", "max", "%16", "%84", "observed", "obs Vs. pred"]
#    print(tabulate(table, headers=heads))
    return sids, agr_cats, agrs, bgrs, miocs, maocs, perc_16s, perc_84s, obss, pf
