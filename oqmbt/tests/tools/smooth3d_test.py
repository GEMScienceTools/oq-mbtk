...
from mbt.tools.smooth3d import Smoothing3D
...
class Smooth3DTestCase(unittest.TestCase):
    ...
    def testcase01(self):
        ...
        if False:
            ...
            #ax.plot(catalogue.data['longitude'], catalogue.data['latitude'],
            #        catalogue.data['depth'], 's', alpha=0.1)
            ax.plot(self.mesh.lons, self.mesh.lats, self.mesh.depths, '.k',
                    alpha=0.2)
            ax.scatter(self.mesh.lons[iii], self.mesh.lats[iii],
                       self.mesh.depths[iii], c=np.log10(values[iii]),
                       alpha=0.5)
            ax.invert_zaxis()
            plt.show()
